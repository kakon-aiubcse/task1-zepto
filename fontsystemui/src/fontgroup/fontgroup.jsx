...
const Fontgroup = () => {
  ...
  const handleDelete = async (id) => {
    try {
      const response = await fetch(`http://localhost:8080/api/delete-group/${id}`, {
        method: 'DELETE',
      });
      if (response.ok) {
        setFontGroups((prevGroups) => prevGroups.filter((group) => group.id !== id));
  ...
  return (
    <div className="flex flex-col ml-60 w-4/6 p-5 pb-10">
      <div className="flex flex-col items-start justify-start mt-4 ">
        <h1 className="text-3xl font-bold text-gray-800 mb-2">Our Font Group</h1>
      ...
      <div className="font-group-container">
        ...
        <table className="min-w-full table-auto border-collapse">
          <thead>
            <tr>
              <th className="px-4 py-2 text-center border-b font-semibold">Name</th>
              <th className="px-4 py-2 text-center border-b font-semibold">Fonts</th>
              <th className="px-4 py-2 text-center border-b font-semibold">Count</th>
              <th className="px-4 py-2 text-center border-b font-semibold"></th>
            </tr>
          </thead>
          <tbody>
            {fontGroups.length > 0 ? (
              fontGroups.map((group) => (
                <tr key={group.id} className="border-b text-center">
                  <td className="px-4 py-2">{group.group_name}</td>
                  <td className="px-4 py-2 items-center">
                    {JSON.parse(group.font_names)}
                  </td>
                  <td className="px-4 py-2 items-center">
              ...
              <tr>
                <td colSpan="3" className="text-center px-4 py-2">No font groups available.</td>
              </tr>
            )} {msg && (
                <p className="my-4 p-4 text-base text-blue-900 font-medium">{msg}</p>
              )}
          </tbody>
        </table>
      </div>
